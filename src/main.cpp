...
static void usage(const char* prog) {
    std::cerr << "Usage: " << prog << " <subcommand> [options]\n"
              ...
              << "  split       Classify reads as damaged/undamaged (no deduplication)\n"
              ...
              << "  merge       Overlap-detect and merge paired-end reads (aDNA-optimised)\n"
...
int main(int argc, char** argv) {
    ...
    if (sub == "split")                      return split_main(argc - 1, argv + 1);
    if (sub == "profile")     return profile_main(argc - 1, argv + 1);
    ...
    if (sub == "merge")       return merge_main(argc - 1, argv + 1);
